...
class Event:
    def __init__(
        ...
        self.model_noise = noise_level / 100 * coeffs_norm
        ...
        low_rank = (1 - tail_strength) * np.exp(
            -1.0 * (singular_ind / self.effective_rank) ** 2
        )
    ...
    def sample(self, n_samples=100):
        ...
        X = X[:n_samples, :]
        y = np.dot(X, self.coefficients) + self.model_noise * np.random.randn(n_samples)
